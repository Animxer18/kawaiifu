...
export function useAnimeWatch(id) {
  ...
  async function getAnimeWatch() {
    ...
    const response = await axios.get(
      `https://animxer-api-cvxg.vercel.app/meta/anilist/info/${id}?provider=${provider}`,
    );
    ...
    if (response.data.episodes.length === 0) {
      ...
      const watchResponse = await axios.get(
        `https://animxer-api-cvxg.vercel.app/meta/anilist/watch/${getWatchId}`,
      );
...
export function useEpisodeId(id) {
  ...
  async function getEpisodeId() {
    const response = await axios.get(
      `https://animxer-api-cvxg.vercel.app/meta/anilist/watch/${id}`,
    );
